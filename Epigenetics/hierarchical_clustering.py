...
# among binding domains and chromatin marks (the output of the script compute_correlation.py)
...
X = np.genfromtxt("significant_sample.txt")
...
sns.heatmap(X_sorted, cmap=cmap, norm=norm, vmax=.3, vmin=-.3, square=False, linewidths=0, annot=False,linecolor='white',
    edgecolor='black', cbar=False, xticklabels=False, yticklabels=False, mask=mask, ax=ax)
ax.tick_params(axis='both',labelsize=5,labelbottom=False,labeltop=True,labelleft=False,length=0)
# Uncomment the following line to save the plot
...
X_rand  = np.genfromtxt("control_sample.txt")
rnd_com = noise_centroids(X_rand, N_clust=N_clusters, N_step=100, method=clust_method, chrs=11, ncol=30)
final_centroid = cut_off(centroid, rnd_com, perc_inf=5, perc_sup=95)
